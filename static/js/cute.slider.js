function UAParser(e) {
	...
	this.getBrowser = function(e) {
		return n(e || t, [/(kindle)\/((\d+)?[\w\.]+)/i, /(lunascape|maxthon|netfront|jasmine)[\/\s]?((\d+)?[\w\.]+)/i,
			/(opera\smini)\/((\d+)?[\w\.-]+)/i, /(opera\smobi)\/((\d+)?[\w\.-]+)/i, /(opera).*\/((\d+)?[\w\.]+)/i,
			/(avant\sbrowser|iemobile|slimbrowser)[\/\s]?((\d+)?[\w\.]*)/i, /ms(ie)\s((\d+)?[\w\.]+)/i,
			/(chromium|flock|rockmelt|midori|epiphany)\/((\d+)?[\w\.]+)/i, /(chrome|omniweb|arora|dolfin)\/((\d+)?[\w\.]+)/i
		], ["name", "version", "major"], [/android.+crmo\/((\d+)?[\w\.]+)/i], [
			["name", "Chrome"], "version", "major"
		], [/(mobile\ssafari|safari|konqueror)\/((\d+)?[\w\.]+)/i, /(applewebkit|khtml)\/((\d+)?[\w\.]+)/i,
	...
	this.getEngine = function(e) {
		return n(e || t, [/(presto)\/([\w\.]+)/i, /([aple]*webkit|trident)\/([\w\.]+)/i, /(khtml)\/([\w\.]+)/i], ["name",
			"version"
		], [/rv\:([\w\.]+).*(gecko)/i], ["version", "name"])
	};
	this.getOS = function(e) {
		return n(e || t, [/(windows\sphone\sos|windows)\s+([\w\.\s]+)*/i], ["name", ["version", /(nt\s[\d\.]+)/gi, r.os.win]],
			[/(blackberry).+version\/([\w\.]+)/i,
				...
				["name", /.+/g, "iOS"],
				["version", /_/g, "."]
			], [/(mac\sos)\sx\s([\w\s\.]+)/i], ["name", ["version", /_/g, "."]], [/(macintosh|unix|minix|beos)[\/\s]?()*/i], [
				"name", "version"
			])
	};
	this.getDevice = function(e) {
		return n(e || t, [/\((ip[honead]+|playbook);/i, /(blackberry)[\s-]?(\w+)/i,
			/(blackberry|benq|nokia|palm(?=\-)|sonyericsson)[\s-]?([\w-]+)*/i, /(hp)\s([\w\s]+)/i, /(hp).+(touchpad)/i,
			/(kindle)\/([\w\.]+)/i, /(lg)[e;\s-]+(\w+)*/i, /(nintendo|playstation)\s([wids3portable]+)/i
		], ["name", "version"], [/(htc)[;_\s-]+([\w\s]+(?=\))|[\w]+)*/i, /(zte)-([\w]+)*/i], ["name", ["version", /_/g,
			" "
		]], [/\s(milestone|mz601|droid[2x]?|xoom)[globa\s]*\sbuild\//i, /mot[\s-]?(\w+)*/i], [
...
Aroma.Engine = function(e) {
	...
	this.startEff = function() {
		...
		this._part_duration = this._duration / (this._selector.getCount() - (1 - this._overlapping) * (this._selector.getCount() -
			1));
		...
		for (var a = 0, f = this._selector.getCount(); a < f; ++a) {
			...
			for (var l = 0, c = e.length; l < c; ++l) {
				...
				for (var h = 0, p = s.length; h < p; ++h) {
					...
					t = (new TWEEN.Tween(e[l].proxy)).delay(n.delay * 1e3 || 0).to(s[h].to, this._part_duration * s[h].time * 1e3).easing(
						n.ease || TWEEN.Easing.Linear.EaseNone).onUpdate(e[l].proxyUpdate);
					if (h == 0) t.start();
					if (u != null) u.chain(t);
					u = t;
	...
	this.effComp = function() {
		if (this.piece.view.engine.onComplete) this.piece.view.engine.onComplete.listener.call(this.piece.view.engine.onComplete
			.ref)
	}
};
...
Aroma.AbstractView = function(e, t) {
	...
	this.getPieceBounds = function(e, t) {
		...
		if (this.extra_part_width == 0) {
			...
			n.x = e > this.extra_part_width ? (this.part_width + 1) * this.extra_part_width + (e - this.extra_part_width) *
				this.part_width : (this.part_width + 1) * e
		}
		if (this.extra_part_height == 0) {
			...
			n.y = t > this.extra_part_height ? (this.part_height + 1) * this.extra_part_height + (t - this.extra_part_height) *
				this.part_height : (this.part_height + 1) * t
		}
		return n
	};
...
Cute.Effect8.prototype.updateConfig = function() {
	...
	this.pieceOptions.depth = this.depth <= 0 ? this.dir == "u" || this.dir == "d" ? this.piece.bounds.height : this.piece
		.bounds.width : this.depth;
...
(function() {
	...
	window.removeResizeListener = function(e, t) {
		for (var n = 0; n < window.resizeListeners.length; ++n) {
			if (window.resizeListeners[n].listener == e && window.resizeListeners[n].ref == t) window.resizeListeners.splice(n,
				1)
		}
	}
})();
...
Cute.FallBack.isMobileDevice = Cute.FallBack.ua.os.name.toLowerCase() == "android" || Cute.FallBack.ua.os.name.toLowerCase() ==
	"ios" || Cute.FallBack.ua.os.name.toLowerCase() == "windows phone os",
	function() {
...
Averta.ScrollContainer.prototype = {
	...
	setup: function() {
		...
		function i(t) {
			...
			e.element.childNodes[0].style.left = -(t.clientX - r) / (e.element.offsetWidth / (e.element.childNodes[0].offsetWidth -
				e.element.offsetWidth)) + "px"
		}
...
Cute.SlideManager.prototype = {
	...
	prepareTransition: function(e) {
		if (this.rotator.fallBack.getType() == Cute.FallBack.DOM2D) return e.transitions2D[parseInt(Math.random() * e.transitions2D
			.length)];
...
Cute.CircleTimer.prototype.draw = function() {
	...
	this.ctx.beginPath();
	this.ctx.arc(this.__w * .5, this.__w * .5, this.config.radius, Math.PI * 1.5, Math.PI * 1.5 + 2 * Math.PI * this.prog,
		false);
...
Cute.SlideControl.Point.prototype = {
	...
	drawThumb: function() {
		...
		if (this.sc.config.thumb_align == "up") this.thumb.domElement.style.top = 10 - this.thumb.frame.offsetHeight + -
			this.thumb_pos * .1 + "px";
...
Cute.SlideInfo.prototype.show = function() {
	...
	if (this.eff == "slide") this.position = -(this.data.align == "bottom" || this.data.align == "top" ? this.content.offsetHeight :
...
Cute.SlideInfo.prototype.hide = function() {
	...
	if (this.showTween) this.showTween.stop();
	this.showTween = (new TWEEN.Tween(this)).to({
		position: this.eff != "slide" ? 0 : -(this.data.align == "bottom" || this.data.align == "top" ? this.content.offsetHeight :
			this.content.offsetWidth)
...
Cute.VideoControl.prototype.showVideo = function() {
	...
	if (this.video_ele.getAttribute("src") != this.data.getAttribute("href")) this.video_ele.setAttribute("src", this.data
