...
! function() {
	function t(t) {
		...
		for (var e = 0, r = this.data.length; e < r; e++) {
			...
			n > 65536 ? (i[0] = 240 | (1835008 & n) >>> 18, i[1] = 128 | (258048 & n) >>> 12, i[2] = 128 | (4032 & n) >>> 6, i[3] =
					128 | 63 & n) : n > 2048 ? (i[0] = 224 | (61440 & n) >>> 12, i[1] = 128 | (4032 & n) >>> 6, i[2] = 128 | 63 & n) :
				n > 128 ? (i[0] = 192 | (1984 & n) >>> 6, i[1] = 128 | 63 & n) : i[0] = n, this.parsedData.push(i)
		}
	...
	t.prototype = {
		...
		makeImpl: function(t, r) {
			...
			this.setupPositionProbePattern(0, 0), this.setupPositionProbePattern(this.moduleCount - 7, 0), this.setupPositionProbePattern(
					0, this.moduleCount - 7), this.setupPositionAdjustPattern(), this.setupTimingPattern(), this.setupTypeInfo(t, r),
				this.typeNumber >= 7 && this.setupTypeNumber(t), null == this.dataCache && (this.dataCache = e.createData(this.typeNumber,
					this.errorCorrectLevel, this.dataList)), this.mapData(this.dataCache, r)
		},
		...
		createMovieClip: function(t, e, r) {
			...
			for (var n = 0; n < this.modules.length; n++)
				for (var o = 1 * n, a = 0; a < this.modules[n].length; a++) {
					var s = 1 * a;
					this.modules[n][a] && (i.beginFill(0, 100), i.moveTo(s, o), i.lineTo(s + 1, o), i.lineTo(s + 1, o + 1), i.lineTo(
						s, o + 1), i.endFill())
				}
			return i
		},
		...
		for (var v = new Array(p), w = 0, m = 0; m < n; m++)
			for (h = 0; h < e.length; h++) m < a[h].length && (v[w++] = a[h][m]);
		for (m = 0; m < o; m++)
			for (h = 0; h < e.length; h++) m < s[h].length && (v[w++] = s[h][m]);
		return v
	};
	for (var s = {
			MODE_NUMBER: 1,
			...
			getBCHTypeInfo: function(t) {
				for (var e = t << 10; u.getBCHDigit(e) - u.getBCHDigit(u.G15) >= 0;) e ^= u.G15 << u.getBCHDigit(e) - u.getBCHDigit(
					u.G15);
				return (t << 10 | e) ^ u.G15_MASK
			},
			getBCHTypeNumber: function(t) {
				for (var e = t << 12; u.getBCHDigit(e) - u.getBCHDigit(u.G18) >= 0;) e ^= u.G18 << u.getBCHDigit(e) - u.getBCHDigit(
					u.G18);
				return t << 12 | e
			},
				...
				for (var e = t.getModuleCount(), r = 0, i = 0; i < e; i++)
					for (u = 0; u < e; u++) {
						for (var n = 0, o = t.isDark(i, u), a = -1; a <= 1; a++)
							if (!(i + a < 0 || e <= i + a))
								for (var s = -1; s <= 1; s++) u + s < 0 || e <= u + s || 0 == a && 0 == s || o == t.isDark(i + a, u + s) && n++;
						n > 5 && (r += 3 + n - 5)
					}
				for (i = 0; i < e - 1; i++)
					for (u = 0; u < e - 1; u++) {
						var h = 0;
						t.isDark(i, u) && h++, t.isDark(i + 1, u) && h++, t.isDark(i, u + 1) && h++, t.isDark(i + 1, u + 1) && h++, 0 !=
							h && 4 != h || (r += 3)
					}
				for (i = 0; i < e; i++)
					for (u = 0; u < e - 6; u++) t.isDark(i, u) && !t.isDark(i, u + 1) && t.isDark(i, u + 2) && t.isDark(i, u + 3) &&
						t.isDark(i, u + 4) && !t.isDark(i, u + 5) && t.isDark(i, u + 6) && (r += 40);
				for (u = 0; u < e; u++)
					for (i = 0; i < e - 6; i++) t.isDark(i, u) && !t.isDark(i + 1, u) && t.isDark(i + 2, u) && t.isDark(i + 3, u) &&
						t.isDark(i + 4, u) && !t.isDark(i + 5, u) && t.isDark(i + 6, u) && (r += 40);
				for (var l = 0, u = 0; u < e; u++)
					for (i = 0; i < e; i++) t.isDark(i, u) && l++;
	...
	for (f = 8; f < 256; f++) c.EXP_TABLE[f] = c.EXP_TABLE[f - 4] ^ c.EXP_TABLE[f - 5] ^ c.EXP_TABLE[f - 6] ^ c.EXP_TABLE[
		f - 8];
	...
	r.prototype = {
		...
		for (var n = r.length / 3, o = [], a = 0; a < n; a++)
			for (var s = r[3 * a + 0], h = r[3 * a + 1], l = r[3 * a + 2], u = 0; u < s; u++) o.push(new i(h, l));
		return o
	...
	var d = [
		...
		g = function() {
			...
			return t.prototype.draw = function(t) {
				...
				o.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:xlink", "http://www.w3.org/1999/xlink"), i.appendChild(o),
		...
		function() {
			...
			var i = function(t, e) {
				this._bIsPainted = !1, this._android = o(), this._htOption = e, this._elCanvas = document.createElement("canvas"),
					...
					this._oContext = this._elCanvas.getContext("2d"), this._bIsPainted = !1, this._elImage = document.createElement(
						"img"), this._elImage.alt = "Scan me!",this._elImage.className="codeImg", this._elImage.style.display = "none", this._el.appendChild(this._elImage),
	...
	(QRCode = function(t, e) {
		...
		"string" == typeof t && (t = document.getElementById(t)), this._htOption.useSVG && (p = g), this._android = o(),
			this._el = t, this._oQRCode = null, this._oDrawing = new p(this._el, this._htOption), this._htOption.text && this.makeCode(
				this._htOption.text)
...
function(t, e, r) {
		...
		i.imageSelector && (i.image = n(i.imageSelector).map(function(t) {
			...
			function(t, e) {
				...
				h(n ? i.reverse() : i, function(i) {
					...
					h[0].href = o, "wechat" === i ? h[0].tabindex = -1 : h[0].target = "_blank", e.initialized || (n ? t.insertBefore(
						h[0], t.firstChild) : t.appendChild(h[0]))
				})
			}(t, i),
			function(t, e) {
				var r = a(t, "icon-wechat", "a");
				if (0 === r.length) return !1;
				var i = s('<div class="wechat-qrcode"><h4>' + e.wechatQrcodeTitle +
	...
	function h(t, e) {
		...
		if (i === r) {
			for (var n in t)
				if (t.hasOwnProperty(n) && !1 === e.call(t[n], t[n], n)) break
		} else
	...
	var l = Array.prototype.indexOf,
		...
		v = {
			...
			sites: ["weibo", "qq", "wechat", "tencent", "douban", "qzone", "linkedin", "diandian", "facebook", "twitter",
				"google"
			],
			mobileSites: [],
			disabled: [],
